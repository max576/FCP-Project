...
#Controls consisting of the self functions defined above
def initialize_controls(controls):
    
    #initialize option for control list parameters
...
def initialize_periods(periods):
    ...
    #import option for period content consisting of the self functions defined above
...
#Functions for hover feature - activates when mouse is over each checkbox and chnages depending on which control
def on_enter(event):
...
#Resets label to blank when mouse leaves checkbox
def on_leave(event):
    label4.configure(text="")    
    
#function that plots the graph and changes it for each period
def plot_and_move_next_period(fig, canvas, cb_var):
   #plot chart for this period
    ...
    #Chnages "next period2 button to "results" and changes the command to open the reults page
...
#gets info from the tkinter window on the current period    
def currPeriod(event=None):
    return current_period.get()

#initialises magic nuber (initial infections)
def magic_number():
    return MAGIC_NUMBER
...
def plot(fig, canvas, userchoice):
    ...
    #overrides the fig in PLOTS folder to be displayed in the results screen
...
def calc_pie():
    ...
    #If user does not select anything a message will pop up telling them to restart
    if int(e) == 0:
        fig, ax = plt.subplots()
        controls = ['']
        colour = ['white']
    ...
    #othereise pie chart of debt will be produced    
    if int(e) > 0:
        fig, ax = plt.subplots()
...
#function that calls help window if user is stuck
def Show_help():
    from SCRIPTS._help import Get_help
    Get_help()
    
#function calls results screen    
def show_results():
    calc_pie()
    window.destroy()
...
#sets up replay button for results screen
def Replay():
